...
import { default as AccordionMenu, structure } from '../AccordionMenu';
...
describe("Accordion Menu", () => {
  ...
  it("renders an AccordionButton element for each variable in the structure array", () => {
    ...
    let items = wrapper.find("AccordionButton");
    expect(items.length).toBe(structure.length);
  })
  

  it("has a button that calls toggleExpanded when clicked", () => {
    let toggleExpanded = jest.genMockFunction();
    ...
    let showHideButton = wrapper.find(".showHideMenuButton");
    expect(showHideButton.length).toBe(1);
